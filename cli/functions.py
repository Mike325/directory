...
def print_to_console(register):
    print(
        "========================================================================================================"
    )
    spaces = get_spaces()
    print(
        "| id | Name{name_space}| Last Name{last_name_space}| Address{address_space}| Cellphone{cellphone_space}| Email{email_space}| Social Network{social_network_space}|"
    ).format(
        name_space=" " * (spaces["name"] - len(translits["name"]) + 1),
        last_name_space=" " *
        (spaces["last_name"] - len(translits["last_name"]) + 1),
        address_space=" " *
        (spaces["address"] - len(translits["address"]) + 1),
        cellphone_space=" " *
        (spaces["cellphone"] - len(translits["cellphone"]) + 1),
        email_space=" " * (spaces["email"] - len(translits["email"]) + 1),
        social_network_space=" " *
        (spaces["social_network"] - len(translits["social_network"]) + 1), )
    for element in elements:
        id_space = " " * (spaces["id"] - len(element["id"]))
        name_space = " " * (spaces["name"] - len(element["name"]))
        last_name_space = " " * (
            spaces["last_name"] - len(element["last_name"]))
        address_space = " " * (spaces["address"] - len(element["address"]))
        cellphone_space = " " * (
            spaces["cellphone"] - len(element["cellphone"]))
        email_space = " " * (spaces["email"] - len(element["email"]))
        social_network_space = " " * (
            spaces["social_network"] - len(element["social_network"]))
        print(
            "| {id} | {name} | {last_name} | {address} | {cellphone} | {email} | {social_network} |"
        ).format(
